...
def write_phonemes(phonemes_dict, phonesfile):
    with open(phonesfile, "w", encoding="utf-8") as fp:
...
def write_extra_questions(nonsil_phonemes, nonsil_phonemes_dict,
    ...
    with open(extra_questions, "w", encoding="utf-8") as fp:
        ...
        for tag in tags:
            for p, p_vals in nonsil_phonemes_dict.items():
                for p_val in p_vals:
                    if ("_" + tag in p_val.strip()):
                        fp.write("%s " % p_val.strip())
            for p, p_vals in sil_phonemes_dict.items():
                for p_val in p_vals:
                    if ("_" + tag in p_val):
                        fp.write("%s " % p_val.strip())
            fp.write("\n")
...
def main():
    ...
    parser.add_argument("--silence-lexicon", help="File with silence words "
                        ...
                        default=None)
    parser.add_argument("--extra-sil-phones", help="Extra silence phones", type=str, default=None)
    ...
    sil_words = []
    try:
        with open(args.silence_lexicon, "r", encoding="utf-8") as fi:
            for line in fi:
    ...
    sil_phonemes = set(sil_phonemes)
    if args.extra_sil_phones is not None:
        for p in args.extra_sil_phones.split():
            sil_phonemes.add(p)
            sil_phonemes_dict[p] = [p]
    sil_phonemes = sorted(sil_phonemes)
    ...
    nonsil_lexicon = []
    try:
        with open(args.lexicon, "r", encoding="utf-8") as fi:
            for line in fi:
    ...
    nonsil_phonemes = set(nonsil_phonemes)
    nonsil_phonemes.difference_update(sil_phonemes)
    nonsil_phonemes = sorted(nonsil_phonemes)
    nonsil_phonemes_dict = {k: v for k, v in nonsil_phonemes_dict.items() if k not in sil_phonemes} 
