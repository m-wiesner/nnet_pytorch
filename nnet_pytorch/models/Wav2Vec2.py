...
class Wav2Vec2(nn.Module):
    ...
    def build_model(cls, conf):
        return Wav2Vec2(
            conf['num_targets'],
            mdl_name=conf['wav2vec2_mdl_name'],
            freeze=conf['wav2vec2_free_encoder']
        )
    ...
    def forward(self, sample):
        ...
        cw_subsampled = len(range(0, cw, 640))
...
class ChainWav2Vec2(Wav2Vec2):
    @classmethod
    def build_model(cls, conf):
        return ChainWav2Vec2(conf['num_targets'], mdl_name=conf['wav2vec2_mdl_name'])
